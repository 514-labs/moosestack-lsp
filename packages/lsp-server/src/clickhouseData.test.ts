...
describe('clickhouseData', () => {
  ...
  describe('loadClickHouseData', () => {
    ...
    it('includes expanded combinator functions and first-word keyword aliases', async () => {
      ...
      const generatedFunction = data.functions.find((f) =>
        f.description.includes('Generated by applying the'),
      );
      assert.ok(
        generatedFunction,
        'Expected at least one function with a generated combinator description',
      );

      const multiWordKeyword = data.keywords.find((k) => k.includes(' '));
      assert.ok(multiWordKeyword, 'Expected at least one multi-word keyword');
      const firstWord = multiWordKeyword.split(' ')[0];
      assert.ok(
        data.keywords.includes(firstWord),
        `Expected first-word keyword alias "${firstWord}" to be present for "${multiWordKeyword}"`,
      );
    });
  });
});
